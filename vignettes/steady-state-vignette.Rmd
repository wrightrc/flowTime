...
read.flowSet('flowSet folder', path = 'your/flow/directory', phenoData = 'annotation.txt', alter.names = TRUE)
```
...
p <- ggplot(dat.SS, aes(as.factor(treatment), FL2.A, fill = AFB)) + geom_boxplot(outlier.size = NA) + facet_grid(IAA~AFB) + theme_classic(base_family = 'Arial', base_size = 16) + ylim(c(-1000,10000)) + xlab(expression(paste('Auxin (',mu,'M)',sep = ""))) + ylab('Fluorescence (AU)') + theme(legend.position="none")
p
```
