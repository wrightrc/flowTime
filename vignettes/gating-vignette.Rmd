...
```{r}
data <- read.flowSet(path = "/Volumes/GoogleDrive/Shared drives/PlantSynBioLab/Data/Mahbub/FlowSets/20200614_AFB_epistasis", phenoData = "annotation.txt")
```
...
autoplot(data[[3]], x = "FSC-A") + geom_gate(nonDebris)
```
...
```{r}
yeast <- gate_flowClust_2d(data.1frame, xChannel = "FSC.A", yChannel =  "SSC.A",                        K = 1, quantile = .95, min = c(0,0))
autoplot(data.1frame, x = "FSC-A", y = "SSC-A") + geom_gate(yeast)

```
To apply this gate across our whole flowSet, we need to either transform the whole flowSet, 
```{r}
yeast <- transform(yeast, inv.trans)
data <- transform(data, inv.trans)
data.1frame <- transform(data.1frame, inv.trans)
```

```{r}
autoplot(data[c(1, 41, 81, 120, 160)], x = "FSC-A", "SSC-A") + geom_gate(yeast)
...
Now we can make a gating template to begin storing all of this gating information in. **Need to finish adding this in**
```{r, gating-template}
gtFile <- system.file("extdata/gating_template/tcell.csv", package = "openCyto")
library(data.table)
gt <- fread(gtFile)
write.csv(gt, file= "gating_template.csv")
```
...
library(flowStats)
chnl <- c("FSC-A", "FSC-H")
singlets <- gate_singlet(x = Subset(data.1frame, yeast), area = "FSC.A",height = "FSC.H", prediction_level = 0.999, maxit = 20)
...
```{r}
autoplot(data[c(1:4, 41:44)], x = "FSC-A", y = "FSC-H") + geom_gate(singlets) + facet_wrap("name", ncol = 4)

autoplot(Subset(data[c(1:4, 41:44)], !nonDebris & singlets), x = "FL1-A") + facet_wrap("name", ncol = 4) 
```
...
This looks quite good. As we might expect as the growth enters exponential phase as time progresses (and well numbers get higher) more of our population is captured in the yeast and singlet gates. Based on the periodicity of this it also looks like there is some dependence on the strain. Let's go ahead and apply these gates to the data.

```{r}
data <- Subset(data, yeast & singlets)
```

Now we can save these as a gateSet. Within the current version of flowTime these gates are just saved as separate objects within a single `.Rdata` file using the saveGates function. The gates we can define within this function are `yeastGate` defining the population of yeast cells from junk, `dipsingletGate` defining the singlets of your diploid yeast strain, `dipdoubletGate` defining the population of diploid doublet cells, and similarly `hapsingletGate` and `hapdoubletGate` for haploid cells.

This example data set is of a diploid strain, so we will only create these gates.
```{r}
